            ...
            <div className='grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 px-10 py-2 text-sm text-zinc-400 border-b border-white/5 bg-zinc-900'>
              {currentAlbum?.songs.map((song) => (
                <div key={song.id} className="relative group">
                  <Card className="flex flex-col items-center p-4 bg-zinc-800 rounded-lg transition-transform duration-300">
                    <img
                      src={song.imageUrl}
                      alt={song.title}
                    ...
                    <p className="text-zinc-400">{formatDuration(song.duration)}</p>
                  </Card>
